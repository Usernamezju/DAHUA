...
function modelReleaseMarkup() {
  const value=state.modelStatus||{}, production=value.production||{}, metrics=production.metrics||{}, candidate=value.candidate, gate=production.release_gate;
...
function renderTrainingStatus() {
  ...
  const statusLabel=running?"正在训练":value.status==="failed"?"训练/发布未通过":value.status==="completed"?"训练与发布完成":"等待新增数据";
  const gate=value.release_gate, gateChecks=gate?.checks||{};
  const gateRows=Object.entries(gateChecks).map(([key,passed])=>`<div><span>${({global_macro_f1:"总体 Macro-F1",new_macro_f1:"新增样本 Macro-F1",old_forgetting:"旧数据遗忘",dangerous_recall:"危险类别召回",calibration:"ECE 校准",edge_size:"部署大小"})[key]||key}</span><strong class="${passed?"gate-pass":"gate-fail"}">${passed?"通过":"未通过"}</strong></div>`).join("");
  const gateMarkup=gate?`<article class="panel training-gate-card"><div class="panel-head"><div><h3>Release Gate</h3><p>${gate.passed?"指标满足发布条件，已切换生产模型":"指标未满足发布条件；campus_all 和生产模型保持不变"}</p></div><span class="status-pill ${gate.passed?"online":"disabled"}">${gate.passed?"PASSED":"REJECTED"}</span></div><div class="gate-grid">${gateRows}</div></article>`:"";
  root.innerHTML=`<div class="training-metrics"><article class="panel"><span>当前状态</span><strong>${statusLabel}</strong></article><article class="panel"><span>本轮新增数据</span><strong>${value.new_sample_count||0} 条</strong></article><article class="panel"><span>当前阶段</span><strong>${({waiting_for_data:"等待新增数据",upload:"冻结、划分并上传",train:"服务器模型训练",merge:"合并 campus_all",validate:"模型评估与 Gate",completed:"训练与发布完成",failed:"失败"})[value.stage]||value.stage||"—"}</strong></article><article class="panel"><span>预计剩余</span><strong>${running?durationText(value.estimated_remaining_seconds):"—"}</strong></article></div><article class="panel training-progress-card"><div class="panel-head"><div><h3>${running?"增量训练进行中":"增量训练状态"}</h3><p>最近训练：${formatTime(value.last_training_at)}${value.model_id?` · 生产模型：${escapeHtml(value.model_id)}`:""}</p></div><span class="status-pill ${running?"online":value.status==="completed"?"online":"disabled"}">${running?`${Math.round((value.progress||0)*100)}%`:value.status==="completed"?"DONE":"IDLE"}</span></div><div class="training-progress"><span style="width:${Math.max(0,Math.min(100,(value.progress||0)*100))}%"></span></div>${value.message?`<p>${escapeHtml(value.message)}</p>`:""}</article>${gateMarkup}${modelReleaseMarkup()}`;
}
