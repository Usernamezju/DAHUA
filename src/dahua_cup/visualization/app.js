...
function renderCapabilities() {
  const names = {video_preview:"视频预览",pose_extraction:"骨架提取",ntu120_inference:"NTU120 推理",campus6_inference:"校园六分类",manual_review:"人工审核",dataset_export:"数据导出",qwen_teacher:"Qwen 教师",incremental_training:"增量训练",model_release:"模型发布",live_camera:"实时摄像头"};
  ...
  [["#run-preview",["video_preview"]],["#run-pose",["video_preview","pose_extraction"]],["#run-full",["video_preview","pose_extraction","ntu120_inference"]],["#run-teacher",["qwen_teacher"]]].forEach(([selector,ids]) => {
...
function renderPipeline() {
  const steps=[["视频导入","服务器文件入库",true],["骨架提取","MediaPipe → NTU25",state.capabilities.pose_extraction?.enabled],["学生预测","ProtoGCN Top-5",state.capabilities.ntu120_inference?.enabled],["难例筛选","质量与不确定性",true],["大模型教师","Qwen 多模态",state.capabilities.qwen_teacher?.enabled],["人工复核","六分类与审计",true],["增量训练","回放与蒸馏",state.capabilities.incremental_training?.enabled],["验证发布","注册、灰度与回滚",state.capabilities.model_release?.enabled]];
...
function renderEvolution() {
  ...
  const statusName={starting:"启动中",watching:"自动监控中",running:"执行中",error:"运行异常",disabled:"未启用"}[value.status]||value.status||"未知";
  badge.textContent=statusName;
  badge.className=`status-pill ${enabled&&value.status!=="error"?"online":"disabled"}`;
  button.disabled=!enabled||running;
  button.title=!enabled?(value.reason||"自动闭环未启用"):running?"已有扫描或训练正在运行":"";
  ...
  const reasonName={
    ...
    missing_ntu120_3d_base_annotation:"缺少匹配的 NTU120 3D XSub 标注文件",
...
async function triggerEvolution() {
  try{
    const value=await api("/api/evolution/run",{method:"POST"});
    if(!value.accepted){toast(value.reason||"已有自动闭环任务正在运行",true);return;}
    toast("已触发伪标签收集、难例挖掘和训练条件检查");
...
function renderModels() {
  const models=[
    ["MediaPipe Pose","33 → NTU25 骨架","姿态模型",state.capabilities.pose_extraction?.enabled,"DAHUA_MEDIAPIPE_MODEL"],
    ["ProtoGCN NTU120","官方 XSub Bone 权重","联调模型",state.capabilities.ntu120_inference?.enabled,"DAHUA_PROTOGCN_PRETRAINED"],
    ["ProtoGCN Campus6","校园六分类微调模型","正式模型",false,"等待微调权重"],
    ["Qwen3-VL Teacher","多模态语义教师","服务器教师",state.capabilities.qwen_teacher?.enabled,"等待服务配置"],
  ];
...
function renderSettings() {
  const fields=[["代码根目录",state.system.repository_root],["服务器数据根目录",state.system.data_root],["运行数据目录",state.system.runtime_root],["候选视频目录",state.system.source_root],["浏览器上传目录",state.system.upload_root],["审核清单",state.system.manifest_path],["审核数据库",state.system.database_path],["推理产物目录",state.system.artifact_root],["自动闭环配置",state.system.evolution_config],["当前学生权重",state.system.active_student_checkpoint]];
  $("#system-paths").innerHTML=fields.map(([name,value])=>`<div class="setting-item"><span>${name}</span><code>${escapeHtml(value||"—")}</code></div>`).join("");
  renderGpuSettings();
  $("#environment-help").textContent=`export DAHUA_CODE_ROOT=/workspace/code/DAHUA\nexport DAHUA_DATA_ROOT=/workspace/data/xzz_data/DAHUA\nexport DAHUA_VIS_RUNTIME_ROOT=/workspace/data/xzz_data/DAHUA/runtime/visualization\nexport DAHUA_VIS_SOURCE_ROOT=/workspace/data/xzz_data/DAHUA/datasets/vedio\nexport DAHUA_VIS_MANIFEST=/workspace/data/xzz_data/DAHUA/datasets/vedio/manual_label_manifest.csv\nexport DAHUA_AUTO_EVOLUTION=1\nexport DAHUA_AUTO_EVOLUTION_CONFIG=/workspace/code/DAHUA/dahua_cup/configs/campus/auto_evolution_ntu120.yaml\nexport DAHUA_POSE_PYTHON=/workspace/code/envs/llm_env/bin/python\nexport DAHUA_MEDIAPIPE_DELEGATE=cpu\nexport DAHUA_INFERENCE_GPUS=0,1,2,3,4,5,6,7`;
}
...
function renderGpuSettings() {
  ...
  $("#gpu-help").textContent=`MediaPipe 固定使用 CPU，避免无头 EGL 软件渲染；ProtoGCN 使用 CUDA GPU 池轮询，最多同时运行 ${state.system.max_workers||2} 个后台任务。`;
}
